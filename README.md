...
This is a module that shows the current song playing and its primary artist on Plexamp, with a Plexamp-green underline, for people that don't want to set up mpd. If Plexamp is not active, nothing is shown. If the song name is longer than `trunclen` characers (default 25), it is truncated and `...` is appended. If the song is truncated and contains a single opening parenthesis, the closing paranethsis is appended as well.
...
format-prefix = " "
format = <label>
exec = python /path/to/plexamp/script -f '{artist}: {song}'
...
``` ini
exec = python /path/to/plexamp/script -t 42
```
...
``` ini
exec = python /path/to/plexamp/script -f '{play_pause} {song} - {artist}'
```
...
``` ini
exec = python /path/to/plexamp/script -p '[playing],[paused]'
```
...
Override example:
```ini
exec = python /path/to/plexamp/script --font=1
```
...
Override example:
``` ini
exec = python /path/to/plexamp/script -p '[playing],[paused]' --playpause-font=2
```
